 ...
 * @param reference to a logger object. Note: assumes logger has an .info , .warn, .error function.
 */
var ServerConfig = function (configJsonDirectory, logger) {
  ...
  if (_.isUndefined(this.configDirectory) || _.isEmpty(this.configDirectory) || !fs.existsSync(this.configDirectory)) {
    // print a message.
    util.error('Config directory is invalid.');
  ...
  // create a debug flag and set it to false initially.
  this.debug = false;
...
ServerConfig.prototype.getConfigFilePath = function(configName) {
  ...
  if (fs.existsSync(tempPath)) {
    ...
    // debug print.
    if (this.logger) {
      this.logger.info('Loading config file: ' + fileName);
    }
    else {
      util.log('Loading config file: ' + fileName);
    }
  }
  else {
    if (this.logger) {
      this.logger.error('Failed to load config file for configuration: ' + configName);
    }
    else {
      util.error('Failed to load config file for configuration: ' + configName);
    }
  }
...
ServerConfig.prototype.loadEnvironmentConfig = function(defaultConfig) {
  ...
  if (process.env.NODE_ENV) {
    ...
    if (process.argv.length > 2) {
      ...
      if (this.logger) {
        this.logger.info('Running as: ' + configName);
      }
      else {
        util.log('Running as: ' + configName);
      }
    }
    else {
      // set the config name.
      ...
      if (this.logger) {
        this.logger.info('Running as: ' + defaultConfig);
      }
      else {
        util.log('Running as: ' + defaultConfig);
      }
    }
  }
...
ServerConfig.prototype.setDelimiter = function(delimiter) {
  if (_.isUndefined(delimiter) || _.isNull(delimiter) || _.isEmpty(delimiter) || !_.isString(delimiter)) {
    if (this.logger) {
      this.logger.error('Invalid delimiter specified.');
    }
    else {
      util.error('Invalid delimiter specified.');
    }
